...
WorkingStorageToken = 'working-storage'i !IdentifierPart
LinkageToken        = 'linkage'i         !IdentifierPart
SectionToken        = 'section'i         !IdentifierPart
...
ReferenceToken      = 'reference'i       !IdentifierPart
...
PictureSegment
        = char:PictureChar '(' _ size:PictureSize _ ')' {
                return { char, size }
        }
        / chars:PictureCharSs {
                return { char: 'S', size: chars.length }
        }
        / chars:PictureCharXs {
...
ProcedureUsings
        = UsingToken _ head:Identifier tail:(_ ','? _ Identifier)* {
                return buildList(head, tail, 3)
        }

ProcedureReturnings
        = ReturningToken _ head:Identifier tail:(_ ','? _ Identifier)* {
                return buildList(head, tail, 3)
        }
...
CallUsingImplicits
        = head:CallUsingImplicit tail:(_ ','? _ CallUsingImplicit)* {
                return buildList(head, tail, 3)
        }
...
CallUsingExplicitss
        = head:CallUsingExplicits tail:(_ CallUsingExplicits)* {
                return buildList(head, tail, 1)
        }

CallUsingExplicits
        = CallUsingExplicitByRefs
        / CallUsingExplicitByContents

CallUsingExplicitByRefs
        = ByToken _ ReferenceToken _ head:CallUsingExplicitByRef tail:(_ ','? _ CallUsingExplicitByRef)* {
                return buildList(head, tail, 3)
        }
